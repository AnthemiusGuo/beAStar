local QianDaoLayerNewCtrller = class("QianDaoLayerNewCtrller",izx.baseCtrller)
function QianDaoLayerNewCtrller:ctor(pageName,moduleName,initParam)
    -- {
--     sec = 49  (number)
--     min = 9  (number)
--     day = 18  (number)
...
-- }
	...
	QianDaoLayerNewCtrller.super.ctor(self,pageName,moduleName,initParam);
end
function QianDaoLayerNewCtrller:initTimeInfo(t)
    print("QianDaoLayerNewCtrller:initTimeInfo")
...
function QianDaoLayerNewCtrller:run()
    print("QianDaoLayerNewCtrller:run")
--gBaseLogic.lobbyLogic.pong_now
	...
	QianDaoLayerNewCtrller.super.run(self);
...
function QianDaoLayerNewCtrller:resetData(signin_info)
...
function QianDaoLayerNewCtrller:buQianKaPayResult(event)
...
function QianDaoLayerNewCtrller:pt_lc_send_user_data_change_not(event)
    print("QianDaoLayerNewCtrller:pt_lc_send_user_data_change_not")
...
function QianDaoLayerNewCtrller:requestHTTGongGao()
    -- 强推公告 todo
    print("QianDaoLayerNewCtrller:requestHTTGongGao")
    ...
    gBaseLogic:HTTPGetdata(url,0,function(event)
        print("QianDaoLayerNewCtrller:requestHTTGongGao result")
...
function QianDaoLayerNewCtrller:pt_cl_get_user_signin_days_req()
...
function QianDaoLayerNewCtrller:pt_lc_get_user_signin_days_ack(event)
...
function QianDaoLayerNewCtrller:getAwardDays()
...
function QianDaoLayerNewCtrller:getItemUrl(index) 
    local url = ""
...
function QianDaoLayerNewCtrller:getItemNumber(index) 
    local num = 0
    for k,v in pairs(self.data.ply_items) do 
...
function QianDaoLayerNewCtrller:setItemNumber(ply_items, index,num) 
...
function QianDaoLayerNewCtrller:getUnSigninDays()
    local num = 0
...
function QianDaoLayerNewCtrller:getReSigninDay()
    local day = 0
    for k,v in pairs(self.data.signin_days) do 
        if v == 1  and k < self.data.curday then 
            day = k
...
function QianDaoLayerNewCtrller:getMaxContinuedSigninDays()
    local num = 0
    local max = 0
    for k,v in pairs(self.data.signin_days) do 
...
function QianDaoLayerNewCtrller:getContinuedSigninDays()
    local num = 0
    for k,v in pairs(self.data.signin_days) do 
        if k <= self.data.curday then 
            if v == 2 then 
...
function QianDaoLayerNewCtrller:getQianDaoState(day)
...
function QianDaoLayerNewCtrller:setQianDaoState(day,state)
    print("setQianDaoState:",day,state)
    self.data.signin_days[day] = state
end

function QianDaoLayerNewCtrller:getAwardState(day)
...
function QianDaoLayerNewCtrller:setAwardState(day,state)
    for k,v in pairs(self.data.signin_award) do 
        if v.days_ == day then 
            v.state_ = state
            return
...
function QianDaoLayerNewCtrller:getAwardInfo(day)
    if #self.data.signin_award == 0 then 
        return {}
    end 
    for k,v in pairs(self.data.signin_award) do 
...
function QianDaoLayerNewCtrller:checkDayRow()
    local x = 0
...
function QianDaoLayerNewCtrller:pt_cl_set_user_signin_days_req(sday)
...
function QianDaoLayerNewCtrller:pt_lc_set_user_signin_days_ack(event)
...
function QianDaoLayerNewCtrller:pt_cl_get_user_signin_award_req(days)
...
function QianDaoLayerNewCtrller:pt_lc_get_user_signin_award_ack(event)
...
function QianDaoLayerNewCtrller:updataPlyLobbyData(award, type) 
...
function QianDaoLayerNewCtrller:findPayItemData(idx)
...
function QianDaoLayerNewCtrller:getQianDaoKaPayInfo()
    print("QianDaoLayerNewCtrller:getQianDaoKaPayInfo")
    self.data.payitemslist = gBaseLogic.lobbyLogic.paymentItemList;
...
function QianDaoLayerNewCtrller:getAwardMoney(day) 
...
function QianDaoLayerNewCtrller:getJinDuScale()
...
--获取指定年月的天数和该天的星期
function QianDaoLayerNewCtrller:getDayAndWeek(year, month, day)       
...
function QianDaoLayerNewCtrller:requestHTTPNoticeList()
	if self.NoticeList==nil then
			
		print("QianDaoLayerNewCtrller:requestHTTPNoticeList(")
	    local sign = crypto.md5("gameid="..MAIN_GAME_ID.."&key=232b969e8375");
	    local pdata = {
	                    gameid = MAIN_GAME_ID,
	                    sign = sign
	            }
	    print(URL.NoticeList.."?gameid="..MAIN_GAME_ID.."&sign="..sign)
	    self.NoticeList = {};
	    HTTPPostRequest(URL.NoticeList,pdata,function(event)
	    	if event~=nil and event.notice~=nil then

		    	print("QianDaoLayerNewCtrller:requestHTTPNoticeList(")
		        var_dump(event,4);
		        self.NoticeList = event.notice;
		        self.view.newGonggao:setVisible(false);
		        self.view:initTableView(0,event.notice)
		    end
	        end)
	end
end 
function QianDaoLayerNewCtrller:requestHTTPActiveImg()
	if self.ActiveImg==nil then
		local pdata = {
	                    gameid = MAIN_GAME_ID,
	            }
	    self.ActiveImg = true;
	    print("QianDaoLayerNewCtrller:requestHTTPActiveImg")
	    print(URL.ActivityImg)
	    HTTPPostRequest(URL.ActivityImg,pdata,function(event)
	    	if event~=nil then
		        ...
		        for k,v in pairs(event) do
		        	-- "position":"2","link":"www.tmall.com#","image":"http://iface.b0.upaiyun.com/e2c91f25-60a6-4509-beda-ab8a7c3e8f76.png!p300"
		        	info["pos"..v.position] = v;
		        end
		        self.ActiveImgList = info;
		        var_dump(info,4)
		        self.view:initActiveImg(info)
		        -- self.NoticeList = event.notice
		        -- self.view:initTableView(0,event.notice)
		    end
	        end)
