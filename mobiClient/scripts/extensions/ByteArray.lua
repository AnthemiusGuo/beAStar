...
function ByteArray:writeUChar(__char)
	-- if (__val<0) then
	-- 	__val = 0-(__val - 127);
	-- end
	if (__char>127) then
...
function ByteArray:writeRawByteNoChangePos(__rawByte)
	if self._pos_no_use > #self._buf+1 then
		for i=#self._buf+1,self._pos_no_use-1 do
			self._buf[i] = string.char(0)
		end
	end
	self._buf[self._pos_no_use] = __rawByte
	self._pos_no_use = self._pos_no_use + 1
	return self
end
...
function ByteArray:writeBufNoChangePos(__s)
	for i=1,#__s do
		self:writeRawByteNoChangePos(__s:sub(i))
	end
	return self
end
